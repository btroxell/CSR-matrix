...
CSR_STRUCTURE create_transpose_matrix(CSR_STRUCTURE &csr_matrix)
{
	...
	for (int i = 0; i < transpose_matrix.rows_ + 1; i++)
	{
	 ...
	 //std::cout << transpose_matrix.rows_ << std::endl;
	...
	for (int i = 0; i < transpose_matrix.rows_ + 1; i++)
	{
		...
		//std::cout << "i = " << i << " ith + i+1th index: " << transpose_matrix.row_ptr_[i - 1] << " + " << transpose_matrix.row_ptr_[i] << std::endl;
	}

	//std::cout << transpose_matrix.row_ptr_[transpose_matrix.rows_] << std::endl;
	//std::cout << transpose_matrix.row_ptr_[transpose_matrix.rows_-30] << std::endl;
	/*populates the new csr matrix with the transpose information*/
	for (int i = 0; i < csr_matrix.rows_; i++)
	{
		//std::cout << "i = " << i << ", transpose.row_ptr[i] = " << transpose_matrix.row_ptr_[i] << std::endl;
		for (int j = csr_matrix.row_ptr_[i]; j < csr_matrix.row_ptr_[i + 1]; j++)
		{
			//std::cout << "i = " << i << ", csr.row_ptr[i+1] = " << csr_matrix.row_ptr_[i+1] << std::endl;
			int i2 = csr_matrix.col_ptr_[j];
			//std::cout << "i2 = " << i2 << ", transpose.col_ptr[transpose.row[i2] + rowcounts[i2]] = " << transpose_matrix.row_ptr_[i2] << " + " << row_counts[i2] << " = " << transpose_matrix.row_ptr_[i2] + row_counts[i2] << std::endl;
	...
	std::cout << "after copying values" << std::endl;
